...
bool CAppHexGrid::load3DModel(const char * const file_name)
{
	unsigned int model_Vertex_Array_Object = 0;
	
	unloadCurrent3DModel();// Unload any current 3D model
	
	m_p3DModel = C3DModel::load(file_name, getOpenGLRenderer());// Create new 3D object
...
void CAppHexGrid::unloadCurrent3DModel()
{
	if (m_p3DModel != nullptr)
	{	
		unsigned int vao_ID = m_p3DModel->getGraphicsMemoryObjectId();// Free up graphics memory
...
CAppHexGrid::CAppHexGrid() : CAppHexGrid(CGameWindow::DEFAULT_WINDOW_WIDTH, CGameWindow::DEFAULT_WINDOW_HEIGHT)
{
	cout << "Iniciando el constructor sin parametros: CAppHexGrid()" << endl;
}

CAppHexGrid::CAppHexGrid(int window_width, int window_height) : CApp(window_width, window_height), m_p3DModel(nullptr), m_currentDeltaTime{ 0.0 }, m_objectRotation{ 0.0 }, m_objectPosition{ 0.0f, 0.0f, 0.0f }, m_rotationSpeed{ DEFAULT_ROTATION_SPEED }
{
	cout << "Iniciando el constructor con parametros de tamaño definido: CAppHexGrid()" << endl;
}
...
void CAppHexGrid::update(double delta_Time)
{
	...
	m_currentDeltaTime = delta_Time;// Save current delta time
	...
	//m_objectRotation += degrees_To_Rotate;// accumulate rotation degrees
...
void CAppHexGrid::render()
{
	...
	else // Otherwise, render active object if loaded (or test cube if no object is loaded)
	{
		...
		float color[3] = { 255.0f, 164.0f, 32.0f };
...
void CAppHexGrid::executeMenuAction()
{
	if (getMenu() != nullptr)
	{
		...
		case 1:
			...
			break;
		case 2:
			cout << "<MENU OPTION NOT IMPLEMENTED>" << endl;// Not implemented
			break;
		case 3:
...
void CAppHexGrid::onMouseMove(float delta_X, float delta_Y)
{
